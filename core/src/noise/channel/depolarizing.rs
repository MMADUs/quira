...
use crate::noise::NoiseChannelOperation;
use crate::operations::singleq::{Identity, PauliX, PauliY, PauliZ};
use crate::ops::QuantumGate;
use crate::qstate::density::matrix::Density;
...
impl Depolarizing {
    ...
    fn global_pauli_operators(num_qubits: usize) -> Vec<Matrix<Complex>> {
        let temp_target = QuantumBit::new(0);
        let single_paulis = vec![
            Identity::new().unitary_matrix(),
            PauliX::new(&temp_target).unitary_matrix(),
            PauliY::new(&temp_target).unitary_matrix(),
            PauliZ::new(&temp_target).unitary_matrix(),
        ];
...
impl NoiseChannelOperation for Depolarizing {
    ...
    fn global(&self, num_qubits: usize) -> Self {
        ...
        // construct channel
        let kraus_ops = if self.prob.abs() < EPSILON {
            // Only Identity operator
            vec![Identity::new().unitary_matrix()]
        } else {
            let pauli_ops = Self::global_pauli_operators(num_qubits);
            Self::construct_kraus_operators(num_qubits, self.prob, &pauli_ops)
        };

        let mut new_self = self.clone();
        new_self.num_qubits = num_qubits;
        new_self.kraus_ops = kraus_ops;
        new_self
    }

    /// new mixed depolarizing channel that applies to specific qubits
    fn selective(&self, total_qubits: usize, target_qubits: &[QuantumBit]) -> Self {
        // qubit indexes
        let target_qubits_idx: Vec<usize> = target_qubits.iter().map(|qubit| qubit.index()).collect();
        let target_qubits_idx: &[usize] = &target_qubits_idx;
        // validate
        assert!(!target_qubits_idx.is_empty(), "target qubits cannot be empty");
        assert!(
            target_qubits_idx.iter().all(|&q| q < total_qubits),
        ...
        let kraus_ops = if self.prob.abs() < EPSILON {
            ...
            let num_target_qubits = target_qubits.len();
            let pauli_ops = Self::selective_pauli_operators(total_qubits, target_qubits_idx);
            Self::construct_kraus_operators(num_target_qubits, self.prob, &pauli_ops)
        };

        let mut new_self = self.clone();
        new_self.num_qubits = total_qubits;
        new_self.kraus_ops = kraus_ops;
        new_self
    }
}
